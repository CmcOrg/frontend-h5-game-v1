...
import {
    GameUserExpDeleteByIdSet,
    GameUserExpDO,
    GameUserExpPage,
    GameUserExpPageDTO
} from "@/api/admin/GameUserExpController";
...
export default function () {
    ...
    return (
        <>
            <ProTable<GameUserExpDO, GameUserExpPageDTO>
                ...
                request={(params, sort, filter) => {
                    return GameUserExpPage({...params, sort})
                }}
                tableAlertOptionRender={({selectedRowKeys, selectedRows, onCleanSelected}) => (
                    <Space size={16}>
                        <a className={"red3"} onClick={() => {
                            ExecConfirm(() => {
                                return GameUserExpDeleteByIdSet({idSet: selectedRowKeys as number[]}).then(res => {
