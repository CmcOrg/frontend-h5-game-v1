...
import {
    ...
    GameRoomConfigDO,
    GameRoomConfigInfoById,
    GameRoomConfigInsertOrUpdate,
    GameRoomConfigInsertOrUpdateDTO,
    GameRoomConfigPage,
    GameRoomConfigPageDTO
...
export default function () {
    ...
    const [useForm] = Form.useForm<GameRoomConfigInsertOrUpdateDTO>();

    const [formVisible, setFormVisible] = useState<boolean>(false);

    const currentForm = useRef<GameRoomConfigInsertOrUpdateDTO>({} as GameRoomConfigInsertOrUpdateDTO)

    return (
        <>
            <ProTable<GameRoomConfigDO, GameRoomConfigPageDTO>
                ...
                request={(params, sort, filter) => {
                    return GameRoomConfigPage({...params, sort})
                }}
                toolbar={{
                    actions: [
                        <Button key={"1"} icon={<PlusOutlined/>} type="primary" onClick={() => {
                            currentForm.current = {} as GameRoomConfigInsertOrUpdateDTO
                            setFormVisible(true)
                        }}>新建</Button>
                    ],
                }}
                tableAlertOptionRender={({selectedRowKeys, selectedRows, onCleanSelected}) => (
                    <Space size={16}>
                        <a className={"red3"} onClick={() => {
                            ExecConfirm(() => {
                                return GameRoomConfigDeleteByIdSet({idSet: selectedRowKeys as number[]}).then(res => {
            ...
            <BetaSchemaForm<GameRoomConfigInsertOrUpdateDTO>
                    ...
                    render: (props, dom) => {
                        return [
                            ...
                            currentForm.current.id ? <Button
                                ...
                                onClick={() => {
                                    ExecConfirm(async () => {
                                        return GameRoomConfigDeleteByIdSet({idSet: [currentForm.current.id!]}).then(res => {
                ...
                request={async () => {
                    ...
                    if (currentForm.current.id) {
                        await GameRoomConfigInfoById({id: currentForm.current.id}).then(res => {
                            currentForm.current = res as GameRoomConfigInsertOrUpdateDTO
                        })
                    }
                ...
                onFinish={async (form) => {
                    await GameRoomConfigInsertOrUpdate({...currentForm.current, ...form}).then(res => {
                        ToastSuccess(res.msg)
                        actionRef.current?.reload()
                    })
                    return true
